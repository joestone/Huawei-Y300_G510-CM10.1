...
status_t AudioHardware::setMasterVolume(float v)
{
    ...
    set_volume_rpc(CAD_HW_DEVICE_ID_HANDSET_SPKR, CAD_HW_DEVICE_ID_HANDSET_MIC, SND_METHOD_VOICE, vol-2, m7xsnddriverfd);
...
status_t AudioHardware::doRouting(AudioStreamInMSM72xx *input)
{
    ...
    if (new_snd_device == -1) {
        ...
        if ((mTtyMode != TTY_OFF) && (mMode == AudioSystem::MODE_IN_CALL) &&
            ...
            ALOGI("Routing audio to Wired Headphone and Speaker\n");
            new_snd_device = SND_DEVICE_STEREO_HEADPHONE_AND_SPEAKER;
        } else if ((outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADSET) &&
                   (outputDevices & AudioSystem::DEVICE_OUT_SPEAKER)) {
            ALOGI("Routing audio to Wired Headset and Speaker (%d,%x)\n", mMode, outputDevices);

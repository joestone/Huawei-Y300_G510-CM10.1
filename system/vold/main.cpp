...
static int process_config(VolumeManager *vm) {
	...
	if (property_get("vold.externalsd.isprimary", value, NULL)
    && !strcasecmp(value, "true")){
    ALOGI ("External sd set to primary");    
    if (!(fp = fopen("/etc/vold_external.fstab", "r"))) {
    ...
    ALOGI ("Internal sd set to primary");
	if (!(fp = fopen("/etc/vold_internal.fstab", "r"))) {
        // no volume added yet, create a AutoVolume object
        // to mount USB/MMC/SD automatically
        if (!vol) {
            // FIXME: should not hardcode the label and mount_point
